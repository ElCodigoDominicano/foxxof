"""
A Simple volatility plugin to check for ASLR protection on all processes

ASLR -- Address Space Layout Randomization,

this wasnt common in the windows XP days ; this became more common around the Vista days.
can be used for finding a leverage in processes for code injection.
...
IMAGE_DLL_CHARATERISTICS_DYNAMIC_BASE = 0X0040
...
def check_aslr(pe):
    ...
    if pe.OPTIONAL_HEADER.DllCharacteristics & IMAGE_DLL_CHARATERISTICS_DYNAMIC_BASE:
        dynamic = True
...
class AslrCheck(interfaces.plugins.PluginInterface):
    ...
    def _generator(self, procs):
        ...
        for proc in procs:
            ...
            try:
                ...
                vol_log.error(f"Process {proc_id}: invalid address {e} in later {e.layer_name}")
                continue
    ...
    def run(self):
        ...
        self.config['nt_symbols'], filter_func = self.crete_pid_filter(self.config.get('pid', None)))
